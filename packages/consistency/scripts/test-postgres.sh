...
repo_dir="$(cd "$consistency_dir/../.." && pwd)"
...
cleanup() {
  ...
  for migration in 0011_consistency.down.sql 0010_api_keys.down.sql 0009_secrets.down.sql 0008_environments.down.sql 0007_projects.down.sql 0006_rbac.down.sql 0005_tenancy.down.sql 0004_auth.down.sql 0003_audit_append_only.down.sql 0002_org_encryption_keys.down.sql 0001_core.down.sql; do
    psql "$TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$database_dir/migrations/$migration"
  done
}
trap cleanup EXIT

for migration in 0001_core.sql 0002_org_encryption_keys.sql 0003_audit_append_only.sql 0004_auth.sql 0005_tenancy.sql 0006_rbac.sql 0007_projects.sql 0008_environments.sql 0009_secrets.sql 0010_api_keys.sql 0011_consistency.sql; do
  psql "$TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -f "$database_dir/migrations/$migration"
done
...
TEST_DATABASE_URL="$TEST_DATABASE_URL" TEST_APP_DATABASE_URL="$TEST_APP_DATABASE_URL" \
  npm run test:integration --workspace @himitsu/consistency
...
echo "consistency drift classes, cache invalidation, dispositions, audit, RLS, and rollback validated"
