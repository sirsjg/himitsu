...
AS $function$
  ...
  WHERE key.prefix = candidate_prefix
    ...
    AND key.created_by_user_id IS NOT NULL
    AND EXISTS (
    ...
    AND EXISTS (
      ...
      WHERE membership.org_id = key.org_id
        AND membership.user_id = key.created_by_user_id
  ...
  RETURNING key.id, key.org_id, key.project_id, key.environment_id,
            key.created_by_user_id, key.name, key.prefix, key.access,
            key.expires_at, key.last_used_at
$function$;

CREATE FUNCTION resolve_api_key_identity(candidate_prefix text) RETURNS TABLE (
  id uuid,
  org_id uuid,
  project_id uuid,
  environment_id uuid
...
SET search_path = pg_catalog, public
AS $function$
  SELECT key.id, key.org_id, key.project_id, key.environment_id
