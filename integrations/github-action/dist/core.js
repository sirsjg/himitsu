...
export class ActionCore {
    ...
    async exportVariable(name, value) {
     ...
     */
    async #writeKeyValue(fileVariable, name, value) {
        const path = this.#runtime.env[fileVariable];
        if (path === undefined || path === "") {
            throw new Error(`${fileVariable} is not set. This action must run on a GitHub Actions runner.`);
        }
